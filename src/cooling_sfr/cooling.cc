 ...
 */
 ...
 double coolsfr::DoCooling(double u_old, double rho, double dt, double *ne_guess, gas_state *gs, do_cool_data *cool_data)
 {
     const double Tmin_K = 50.0;
     const double smoothing_factor = 1.1;
     const double max_cool_fraction = 0.5;
 
     double u = u_old;
 
     // Example mu calculation (replace if you have EOS function)
     double mu = 0.59;  // mean molecular weight for ionized gas, placeholder
     double T = u * 2.0 / 3.0 * (All.UnitPressure_in_cgs / All.UnitDensity_in_cgs) * PROTONMASS / (BOLTZMANN * mu);
 
     if (T < Tmin_K)
         T = Tmin_K * smoothing_factor;
 
     // Recalculate internal energy from T
     ...
     if (fabs(delta_u) > max_cool_fraction * u)
         delta_u = copysign(max_cool_fraction * u, delta_u);
 
     u += delta_u;
 
     double u_floor = Tmin_K * 3.0 / 2.0 * (All.UnitDensity_in_cgs / All.UnitPressure_in_cgs) * BOLTZMANN * mu / PROTONMASS;
     if (u < u_floor)
         u = u_floor;
 ...
 void coolsfr::cool_sph_particle(simparticles *Sp, int i, gas_state *gs, do_cool_data *DoCool)
 {
   ...
   mpi_printf("COOLING: (before) Particle %d BEFORE: utherm=%.3e density=%.3e dt=%.3e\n", Sp->P[i].ID.get(), utherm, dens * All.cf_a3inv, dtime);
   ...
   mpi_printf("COOLING: (after) Particle %d AFTER: utherm=%.3e delta_u=%.3e cooling_rate=%.3e\n", Sp->P[i].ID.get(), utherm, du, (dtime > 0) ? du/dtime : 0.0);
